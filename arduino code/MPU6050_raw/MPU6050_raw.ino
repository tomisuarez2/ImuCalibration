...
/* Defined here desired samplig rate frecuency in Hz */
...
void setup() {
  ...
  /* Set sample frecuency */
  Serial.println("Setting sampling frecuency...");
  uint16_t gyroSampleRateFrecuency;
  if (!mpuDLFPMode | mpuDLFPMode == 7) {
    /* Base sampling rate frecuency in Hz */
    gyroSampleRateFrecuency = 8000;   
  }
  else{
    gyroSampleRateFrecuency = 1000;  
  }
  uint8_t mpuDesiredSampleRateDividerRegister = gyroSampleRateFrecuency/SMPL_RT_FREQ - 1;
  ...
  Serial.println("Computing sample rate frecuency...");
  Serial.print("Fs: ");
  Serial.println(gyroSampleRateFrecuency/(1 + mpuSampleRateDivRegister));
